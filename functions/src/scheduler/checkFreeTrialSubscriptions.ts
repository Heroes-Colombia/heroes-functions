...
import { PLAN1 } from "../constants/app_constants";

export default async function checkFreeTrialSubscriptions() {
  //First we get all the business with subscription_status = free Trial
  ...
  businessesWithActiveSubscription.forEach(async (business) => {
    ...
    if (latestTransactionExpirationDate.toMillis() < Date.now()) {
      ...
      //TODO: define the plan for the free trial
